...
app = FastAPI()
# handler = Mangum(app)
pickle_in = open("pickle/model.pkl","rb")
classifier=pickle.load(pickle_in)

@app.get('/')
def index():
    return {'message': 'I am Malicious URL detector, Created by Agash Uthayasuriyan'}

@app.post('/predict')
def predict_malicious(data:link):
    data = data.dict()
    value=data['url']
    obj = FeatureExtraction(value)
    x = [obj.getFeaturesList()]
   # print(classifier.predict([[variance,skewness,curtosis,entropy]]))
    y_pred =classifier.predict(x)
    non_phishing = round(((classifier.predict_proba(x)[0,1])*100),0)
    if non_phishing < 50:
        ret = "This URL might be a newly deployed site or malicious and is " +str(non_phishing) + " % safe"
    else:
        ret = "This URL is " +str(non_phishing) + " % safe"
    return ret
